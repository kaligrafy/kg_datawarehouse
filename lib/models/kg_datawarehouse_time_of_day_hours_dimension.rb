...
module KgDatawarehouse
  class TimeOfDayHoursDimension < ActiveRecord::Base
    self.table_name = :kg_datawarehouse_time_of_day_hours_dimensions
    self.primary_key = :time_of_day_key
    
    def self.fill
      KgDatawarehouse::TimeOfDayHoursDimension.destroy_all # delete existing data from table
      time_start = Time.parse("2000-01-01 00:00:00 -0000")
      
      ActiveRecord::Base.transaction do
        (0..23).each do |i|
          i_time = time_start+i.hours
          new_time = KgDatawarehouse::TimeOfDayHoursDimension.new
