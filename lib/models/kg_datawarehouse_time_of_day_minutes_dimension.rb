...
module KgDatawarehouse
  class TimeOfDayMinutesDimension < ActiveRecord::Base
    self.table_name = :kg_datawarehouse_time_of_day_minutes_dimensions
    self.primary_key = :time_of_day_key
    
    def self.fill
      KgDatawarehouse::TimeOfDayMinutesDimension.destroy_all # delete existing data from table
      time_start = Time.parse("2000-01-01 00:00:00 -0000")
      
      ActiveRecord::Base.transaction do
        (0..1439).each do |i|
          i_time = time_start+i.minutes
          new_time = KgDatawarehouse::TimeOfDayMinutesDimension.new
